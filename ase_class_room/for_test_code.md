...
実はこの辺がテスト準備が大変な理由に直結します。  
だって考えてみれば当然ですよね？

沢山の小さな機能の集合をテストしようとすれば、個々の機能に対するデータを作らなければいけません。
...
デザインが変わるかもしれないのに、Selemnium でせこせこ書きたいでしょうか？
...
ここに分けて見れば、個別にはテストコード書けそうではありませんか？
...
曰く *「プログラムがバグるのに、テストにバグがないわけないだろう」* と。

もちろんバグります。

しかし、二重チェックなので両方噛み合わないとまずテストは失敗します。気づかないで最後まで行くより良くないでしょうか？
...
これはオブジェクト指向では、依存性が高いと言って、避けるべき設計です。
この辺りはオブジェクト指向設計の本を読むか、設計教育を受けるべきです。
